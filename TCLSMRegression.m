classdef TCLSMRegression < nnet.layer.RegressionLayer
    ...
    methods
        function layer = TCLSMRegression(name, numLabelChannels, MMax)           
            ...
            %layer.numObservMax = numObservMax;
            %layer.numObservCur = 0;
            %layer.numObservCur = 0;
        ...
        function dLdY = backwardLoss(layer, Y, T)
            ...
            %Lt
            %Tt = zeros([mt, rt], "like", T);
            %Tt(T(1,:) > Y(2,1)) = 1;
            %fprintf('back loss Tt T=%f Tt=%f Y2=%f\n', T(1,1), Tt(1,1), Y(2,1));
            ...
            %errSquareDer(2,:) = (- 2. * (Y(1,:) - Y(2,:))) .* ((T(2,:) .* 2. - 1.) .^ 2);
            ...
            %errSquareDer(2+1:2+2*layer.MMax,:) = 0;
